...
void myDrawContours(Mat& src, vector<vector<Point>> contours, bool hasLabel = false) {
	int count = 0;
	for (size_t i = 0; i < contours.size(); i++) {
		...
		rectangle(src, r, green, 2, 8, 0);
		String text = to_string(count) + "-" + to_string(r.area());
		cout << text << endl;
		if (hasLabel) {
			putText(src, text, point4, FONT_HERSHEY_SIMPLEX, 0.5, blue, 2);
		}
	}
}
